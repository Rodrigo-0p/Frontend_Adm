...
import VCATEGORIA from './view';
import './styles/styles.css'

const FormName   = 'CATEGORIA'     ;
const idComp     = FormName;
const TituloList = "Categoria";
...
const CATEGORIA = React.memo(() => {
  ...
  const guardar = async()=>{
    ...
    let row_det         = await Main.GenerarUpdateInsert(update_insert_detalle,url_det,'cod_categoria',[],['cod_categoria'],false,['cod_categoria']);
    ...
    if(updateInsertDet.length > 0 || delete_det.length > 0){
      Main.Request(mainUrl.url_abm,"POST",data).then(async(resp) => {
        ...
        if(resp.data.res >= 1){          
          ...
          Main.message.error({
            content  : resp.data.length > 0 ? resp.data.mensaje : 'Favor intente mas tarde!!',
            className: 'custom-class',
            duration : `${2}`,
            style    : {
                marginTop: '2vh',
  ...
  const getData = async (data = {})=>{
    ...
    try {
      ...
      await Main.Request(mainUrl.url_listar_cab,'POST',data).then(async(info)=>{
        ...
        setTimeout(()=>{
          refDet.current.hotInstance.selectCell(0, 1)
        })        
      });      
    } catch (error) {
      console.error(error);
