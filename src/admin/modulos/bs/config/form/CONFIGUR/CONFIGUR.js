...
const TituloList = "Configuración";
...
const CONFIGUR =  React.memo(() => {
  ...
  const getParamsDetalle = async (idCabecera = false, indexRow = 0)=>{
    ...
    valor.key_cab	    = newKey;
    valor.cod_empresa	= sessionStorage.getItem('cod_empresa');    
    valor.idCabecera  = idCabecera ? idCabecera : Main.nvl(refCab.current.data[indexRow]?.key_cab,refCab.current.data[indexRow]?.cod_configuracion);
    return valor;
  }
  ...
  const guardar = async ()=>{
    ...
    let url_det         = mainUrl.url_get_serialDet+`${sessionStorage.getItem('cod_empresa')}/${keyCabecera}`;
    let row_det         = await Main.GenerarUpdateInsert(update_insert_detalle,url_det,'nro_orden',[],['nro_orden'],false,['cod_configuracion'],row_cab.rowsAux);
    ...
    if(row_cab.insertar || row_cab.actualizar){
      ...
      if((row_cab.insertar || row_cab.actualizar) && (Object.keys(uploadImg).length > 0)){
        ...
        row_cab.updateInsert[0].name_img = `/private/${cod_empresa}/${FormName}/${vname_img}${row_cab.updateInsert[0].cod_configuracion}.${uploadImg.name.split('.')[1]}`
        row_cab.rowsAux[0]['name_img']   = `/private/${cod_empresa}/${FormName}/${vname_img}${row_cab.updateInsert[0].cod_configuracion}.${uploadImg.name.split('.')[1]}`
        ...
        if(resp.data.res >= 1){
          ...
          if(row_cab.actualizar || row_cab.insertar){
            ...
            if(row_cab.insertar){        
              form.setFieldsValue({
                ...form.getFieldsValue(),
                id:row_cab.updateInsert[0].cod_configuracion
              });        
            }            
            ...
            saveImg(row_cab.updateInsert[0].cod_configuracion,false)            
          } 
          if(refCab.current.delete.length > 0){
            getData();
          }else{
            let param           = getParmas(true)
            param.cod_configuracion  = row_cab.rowsAux[0].cod_configuracion
            getData(param);          
          }
          ...
          saveImg(row_cab.updateInsert[0].cod_configuracion).then(()=>{
            let param = getParmas(true)
            param.cod_configuracion  = row_cab.rowsAux[0].cod_configuracion
            getData(param)
          }).catch(()=>{
  ...
  const saveImg = async (cod_configuracion,activarMensaje = true)=> new Promise(async(resolve,reject)=>{
    if(fileList && fileList.length > 0 && fileList[0].uid !== '-1' && (uploadImg.name && uploadImg.name.length > 0)){
      try {
        ...
        let urlImg          = mainUrl.url_post_img+`${vcod_empresa}/${vname_img}${cod_configuracion}.${extemcionImg}`;
  ...
  const getParmas = (retornaNull = false) =>{
    var data = {
      cod_configuracion  : retornaNull ? null : form.getFieldValue('cod_configuracion')  !== undefined ? form.getFieldValue('cod_configuracion')  : null,
    ...
    try {
      ...
      Main.Request(mainUrl.url_listar_cab,'POST',data).then((resp)=>{
        ...
        if(resp.data.length > 0){
          ...
          if(buttonF8) document.getElementById('cod_configuracion').focus();
        }else{
          Main.message.info({
  ...
  const manejaF7 = (e)=>{
    try {
      ...
      setTimeout(()=>{      
        ...
        banRef.current.indice = 0;
        document.getElementById('cod_configuracion').focus();
        document.getElementById("indice").textContent         = "1"
		    document.getElementById("total_registro").textContent = "?";
      })  
    } catch (error) {
  ...
  const handleKeyDown = (e)=>{
    if(e.target.id === 'cod_configuracion' && !banRef.current.manejaF7) e.preventDefault();
  ...
  const leftData = async() => {
    if(!refCab.current.activateCambio){
      ...
      if( banRef.current.id_cabecera !== row.cod_configuracion ) banRef.current.id_cabecera = row.cod_configuracion;
    ...
    if(!refCab.current.activateCambio){
      if(refCab.current.data.length !== 1){
        ...
        if(refCab.current.data[indice] !== undefined){
          if(banRef.current.id_cabecera !== refCab.current.data[indice]?.cod_configuracion && !Main._.isUndefined(refCab.current.data[indice].cod_configuracion)){
            banRef.current.id_cabecera = refCab.current.data[indice]?.cod_configuracion;
    ...
    try {
      ...
      Main.Request(mainUrl.url_activar,'POST',data).then((resp)=>{
        ...
        if(resp.data.res >= 1){
          ...
          let param = getParmas(true)
          param.cod_configuracion  = form.getFieldValue('cod_configuracion');
          getData(param);
        }else{
          Main.message.error({
            content  : resp.data.mensaje.length > 0 ? resp.data.mensaje : 'Favor intente mas tarde!!',
